...
// ManageNetworkBandwidth can set and remove bandwidth rule for a container
func (daemon *Daemon) ManageNetworkBandwidth(create types.BandwidthCreateRequest) (*types.BandwidthCreateResponse, error) {
	...
	if create.Remove == true {
		r1 := &bw.Rules{}
		logrus.Printf(" trying to remove container Bandwidth rule for:%s", create.Container)
